...
{   
    std::cout <<(PATH+path).c_str() <<"\t"<< "无法打开文件" << std::endl;
...
bool HttpResponse::makeContent(std::string path, int code)
{
    this->statusCode = code;
    // printf("%s\n",(PATH+path).c_str());
    // 打开文件以供读取
    int fd = open((PATH+path).c_str(), O_RDONLY);
    if (fd == -1) {
        std::cerr <<(PATH+path).c_str() <<"\t"<< "无法打开文件" << std::endl;
        return false;
    }
