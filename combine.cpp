...
public:
    user_conn(){};
    void init(int epollfd,int fd_,Timer *timer_){
        fd = fd_;
        epoll_event event;
        event.data.fd = fd;
...
public:
    Timer(){user = nullptr;};
    void init(user_conn *user_,std::chrono::steady_clock::time_point expiry_){
        user = user_;
        expiry = expiry_;
    };
    ...
    while (true) {
        ...
        for (int i = 0; i < numEvents; ++i) {
            if (events[i].data.fd == serverSocket) {
                ...
                (users[clientSocket]).init(epollFd,clientSocket,timer);
                timer->user = &users[clientSocket];
